...
interface JinaSearchResponse {
	code?: number;
	status?: number;
	data?: JinaSearchResult[] | JinaSearchEnvelope;
}

function extractJinaResults(raw: JinaSearchResponse): JinaSearchResult[] {
...
export class JinaProvider implements FullProvider {
	...
	async search(query: string, maxResults: number, signal?: AbortSignal): Promise<SearchResponse> {
		...
		const results = normalizeJinaResults(extractJinaResults(raw)).slice(0, maxResults);
